...
- (id) initWithFrame:(CGRect)aFrame minValue:(float)aMinValue maxValue:(float)aMaxValue barHeight:(float)height
{
    ...
    if (self)
	{
		...
		self.minHandle = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"slider_block"] highlightedImage:[UIImage imageNamed:@"slider_block_down"]] autorelease];
		self.minHandle.center = CGPointMake(sliderBarWidth * 0.2, sliderBarHeight * 0.5);
		[self addSubview:self.minHandle];
		
		self.maxHandle = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"slider_block"] highlightedImage:[UIImage imageNamed:@"slider_block_down"]] autorelease];
